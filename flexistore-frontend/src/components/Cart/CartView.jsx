            ...
            <div className="flex flex-col md:flex-row justify-between items-center rounded-box bg-base-100 m-5" key={item.id}>
              <div className="flex gap-4">
                ...
                <div className="flex flex-col">
                  ...
                  <p className="font-bold text-secondary ">Kiválasztott átvevőpont: {" "}
                    {
              ...
              <div className="flex gap-4 mt-4 md:mt-0">
                <button className="px-4 py-2 rounded-sm hover:bg-base-200 focus:outline-hidden" onClick={() => { addToCart(item, item.lockerId) }}>
                ...
                <button className="mr-2 px-4 py-2 rounded-sm hover:bg-base-200 focus:outline-hidden" onClick={() => { removeFromCart(item, item.lockerId) }}>
            ...
            <div className="card-body">
              {cartItems.length > 0 ? (
                <div className="flex flex-col justify-between items-center">
                  <h1 className="text-lg font-bold text-primary">Összesen: {getCartTotal()} Ft</h1>
                  <button className="px-4 py-2 bg-info text-white text-xs font-bold uppercase rounded-sm " onClick={() => { clearCart() }}>
                    Teljes kosár kiürítése
                  </button>
                  <button id="next-to-checkout" onClick={() => openPrivacyInfo()} className="m-2 px-4 py-2 bg-success text-white text-xs font-bold uppercase rounded-sm" >
                    Tovább a pénztárhoz
                  </button>
