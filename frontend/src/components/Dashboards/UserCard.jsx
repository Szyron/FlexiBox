...
function UserCard({ user }) {
    ...
    return (
      <div className="bg-base-100 text-base-800 shadow-lg rounded-2xl p-6 border border-base-300">
        <p className="text-center font-bold text-2xl text-info">Azonosító #{user.id}</p>
        <div className="mt-5 bg-white rounded-2xl p-4 shadow-md border border-base-300">
            ...
            <p className="text-secondary">Létrehozva: {new Date(user.created_at).toISOString().split('T')[0]} {new Date(user.created_at).toISOString().split('T')[1].slice(0, 8)}</p>
        </div>
        <div className="flex flex-row justify-between mt-5">
        <p><button onClick={() => openModal()} className="btn bg-primary text-white">
